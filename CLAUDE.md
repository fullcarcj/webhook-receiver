...
| WhatsApp | `WASENDER_ENABLED`, `WASENDER_API_KEY`, `WASENDER_API_BASE_URL`, `ML_WHATSAPP_TIPO_F_ENABLED`, plantillas E/F en BD o env; ventana silenciosa: `WA_QUIET_HOURS_*`, `WA_QUIET_HOURS_BLOCK_SEND` (`waQuietHours.js`); throttle diario: **`WA_DAILY_CAP`** (default `5` — max mensajes por número por día; subir a `999` para pruebas); reintentos HTTP **429** (Account Protection Wasender): `WASENDER_429_MAX_RETRIES` (default `5`), `WASENDER_429_MIN_WAIT_MS` (default `5200`); anti-spam: `WA_PREVENT_DUPLICATES`, `WA_MAX_REMINDERS_PER_DAY`, tabla `wa_sent_messages_log` (`npm run db:wa-anti-spam`, `waAntiSpam.js`) — `messageType` en `wasender-client` (`CHAT`/`REMINDER`/`MARKETING`/`CRITICAL`); reset manual throttle: `npm run wa-throttle-reset -- +58… | --all` |
...
*Última revisión: 2026-04-11 (v2) — **Users+Roles+Permissions:** JWT Bearer + Cookie HttpOnly + X-Admin-Secret (puente legado); tablas `users`/`user_sessions`/`role_permissions`; 3 roles (SUPERUSER 18 perms / ADMIN 16 / OPERATOR 8); lockout 5 intentos; endpoints `/api/auth/*` y `/api/users/*`; migración `npm run db:users`; `jsonwebtoken` + `bcryptjs` instalados. **Revisión backend Ronda 1:** WMS migrado por primera vez en BD (`warehouses`, `bin_stock`, vistas); corregido `producto_sku→product_sku` y `productos→products` en `wms-bins.sql`; scripts `db:wms`, `db:wms-audit`, `db:wms-all` añadidos; `rejectDuringDowntime` agregado a writes de `/api/catalog` y `/api/users`; `cleanup_expired_sessions()` en `dailyRatesFetch.js` (ya estaba).*
