 ...
 * AI Responder — **Tipo M** (piloto WhatsApp CRM): convención `MESSAGE_TYPE_M` / `prompt_ai_responder_pilot` (prompt en código, ml-message-types.js). AI_RESPONDER_ENABLED=1 activa cola + worker (ciclo 5s); AI_RESPONDER_CONFIDENCE_MIN (default 85); **AI_RESPONDER_FORCE_SEND=1** fuerza envío sin revisión humana ni umbral de confianza (úsalo solo en piloto controlado). Migración npm run db:ai-responder; verify: npm run verify:ai-responder. Monitoreo HTML GET /ai-responder?k= (ADMIN_SECRET; lista de otros monitores ?k= en la misma página). API /api/ai-responder/*. GROQ + WASENDER_API_KEY.
 ...
 * Wasender messages.received: el cuerpo suele traer data.messages como array; payloadParser lo desanida (tests/wasender-payload-parser.test.js). Sin eso fromPhone queda vacío y no corre bienvenida ni CRM.
