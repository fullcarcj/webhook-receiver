 ...
 * Wasender — throttle diario: `WA_DAILY_CAP` = max mensajes por número por día calendario (América/Caracas). **Default 5**. Para pruebas subir a 999; en producción bajar a 5-10. Reset manual: `npm run wa-throttle-reset -- +58…` o `--all`. Tabla `wa_throttle` (migración `db:wa-anti-spam`).
 ...
 * AI Responder — **Tipo M** (piloto WhatsApp CRM): plantilla `AI_RESPONDER_GENERIC_TEMPLATE` + `context_line` (GROQ). `AI_RESPONDER_FORCE_SEND` = switch revisión humana: **on** (1/true/yes/on) = sin cola humana antes de enviar; **off** (0/false/no/off o vacío) = revisión humana si aplica. No evita log si Wasender falla. AI_RESPONDER_ENABLED=1 activa cola + worker. Migración npm run db:ai-responder; verify: npm run verify:ai-responder. Monitoreo GET /ai-responder?k=. GROQ + WASENDER_API_KEY.
