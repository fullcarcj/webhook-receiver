 ...
 * ERP_DASHBOARD_URL: URL absoluta del dashboard Next (p. ej. https://app.tudominio.com/dashboard). GET /admin redirige (302) ahí; /admin-panel muestra franja con el mismo enlace. Si no se define, default local http://localhost:3000/dashboard.
 ...
 * Validación nombre onboarding (Groq): si la IA falla (429, red, etc.) **no** se acepta el texto como nombre por defecto (evita contaminar `customers`). Solo con **`WA_NAME_ALLOW_STATIC_FALLBACK=1`** se recupera el antiguo fallback heurístico sin IA.
 ...
 * fbmp_edge — Facebook Marketplace Personal (extensión Chrome): FBMP_EDGE_ENABLED=1 activa el módulo; FBMP_EDGE_INGEST_SECRET (Bearer largo, copiar en la extensión); migración: npm run db:fbmp-edge. Rutas: POST /api/fbmp-edge/ingest (extensión), GET /api/fbmp-edge/outbox (extensión), POST …/ack|fail, GET /api/fbmp-edge/threads (admin), POST /api/fbmp-edge/threads/:id/reply (operador), GET /api/fbmp-edge/status.
