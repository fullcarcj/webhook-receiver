 ...
 * AI Responder — **Tipo M** (piloto WhatsApp CRM): convención `MESSAGE_TYPE_M` / `prompt_ai_responder_pilot` (prompt en código, ml-message-types.js). AI_RESPONDER_ENABLED=1 activa cola + worker; AI_RESPONDER_CONFIDENCE_MIN (default 85). **AI_RESPONDER_FORCE_SEND=1** solo omite la cola de **revisión humana** cuando la IA ya devolvió `reply_text` pero `needs_human` o confianza baja lo mandarían a revisión; no altera el cálculo de `needsHuman` en el modelo. Migración npm run db:ai-responder; verify: npm run verify:ai-responder. Monitoreo GET /ai-responder?k=. GROQ + WASENDER_API_KEY.
