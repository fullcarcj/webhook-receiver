 ...
 * AI Responder — **Tipo M** (piloto WhatsApp CRM): plantilla `AI_RESPONDER_GENERIC_TEMPLATE` + `context_line` (GROQ). **AI_RESPONDER_FORCE_SEND=1** omite **solo** la cola de revisión humana (`needs_human_review` por `needsHuman` o por falta de `reply_text`); **no** evita `needs_human_review` si falla el envío Wasender. AI_RESPONDER_ENABLED=1 activa cola + worker. Migración npm run db:ai-responder; verify: npm run verify:ai-responder. Monitoreo GET /ai-responder?k=. GROQ + WASENDER_API_KEY.
