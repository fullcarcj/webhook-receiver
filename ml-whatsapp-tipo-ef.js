  ...
  if (prevOk === 0) {
    ...
    if (!resImg.ok) {
      await logTipoE({
        ...
        phone_source: source,
        outcome: "api_error",
        http_status: typeof resImg.status === "number" ? resImg.status : null,
        wasender_msg_id: Number.isFinite(mid1) ? mid1 : null,
        response_body: resImg.bodyText ? resImg.bodyText.slice(0, 8000) : null,
        error_message: resImg.reason ? `policy:${resImg.reason}` : `paso 1 imagen HTTP ${resImg.status}`,
    ...
    if (!resLoc.ok) {
      await logTipoE({
        ...
        phone_source: source,
        outcome: "api_error",
        http_status: typeof resLoc.status === "number" ? resLoc.status : null,
        wasender_msg_id: Number.isFinite(mid2) ? mid2 : null,
        response_body: resLoc.bodyText ? resLoc.bodyText.slice(0, 8000) : null,
        error_message: resLoc.reason ? `policy:${resLoc.reason}` : `paso 2 ubicación HTTP ${resLoc.status}`,
...
async function sendWhatsappToBuyerPhones({
  ...
  for (const p of phones) {
    ...
    await db.insertMlWhatsappWasenderLog({
      ...
      http_status: typeof res.status === "number" ? res.status : null,
      response_body: res.bodyText ? res.bodyText.slice(0, 8000) : null,
      error_message: !res.ok ? (res.reason ? `policy:${res.reason}` : `HTTP ${res.status}`) : null,
      text_preview: preview,
    });
