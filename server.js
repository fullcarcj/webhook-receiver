...
<body>
  ...
  <td>${row.has_media ? "sí" : "no"}</td>
   ...
   <td>${row.media_type === "audio" && row.media_firebase_url ? `<audio controls src="${escapeHtml(row.media_firebase_url)}"></audio>` : row.media_type === "video" && row.media_firebase_url ? `<video controls src="${escapeHtml(row.media_firebase_url)}" style="max-width:200px; max-height:150px;"></video>` : row.media_firebase_url ? `<a href="${escapeHtml(row.media_firebase_url)}" target="_blank" rel="noreferrer">abrir</a>` : "—"}</td>
